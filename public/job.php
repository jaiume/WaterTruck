...
<body>
    <div class="header" style="display:flex; align-items:center; justify-content:space-between;">
        ...
        <div id="nav-links" style="display:none; gap:1rem; font-size:0.875rem;">
            <a href="/truck" class="text-muted text-decoration-none" id="truck-nav-link" style="display:none;">Truck Dashboard</a>
            <a href="/operator" class="text-muted text-decoration-none" id="operator-nav-link" style="display:none;">Operator Dashboard</a>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadNavLinks() {
            try {
                ...
                let hasLinks = false;
                if (user.truck) {
                    document.getElementById('truck-nav-link').style.display = 'inline';
                    hasLinks = true;
                }
                if (user.operator) {
                    document.getElementById('operator-nav-link').style.display = 'inline';
                    hasLinks = true;
                }
                if (hasLinks) navLinks.style.display = 'flex';
            } catch (e) {}
        }
        ...
        async function subscribeToPush(registration) {
            try {
                ...
                // Send subscription to server (unified endpoint)
        ...
        loadConfig().then(() => {
            loadJob();
            loadNavLinks();
            // Initialize push notifications for customers tracking their delivery
